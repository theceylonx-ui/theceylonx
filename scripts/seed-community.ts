...
async function seedTopics() {
  ...
  for (const topic of topicsData) {
    try {
      ...
      console.log(`Topic ${topic.name} might already exist, skipping`);
    }
  }
}
...
async function seedUsers() {
  ...
  for (const userData of usersData) {
    try {
      ...
      console.log(`Error with user ${userData.name}:`, error);
    }
  }
}
...
async function seedQA() {
  ...
  for (const email of ["seed@theceylonx.com", "traveler1@theceylonx.com", "traveler2@theceylonx.com", "traveler3@theceylonx.com"]) {
    try {
      const users = await storage.getUserByEmail?.(email);
      if (users) allUsers.set(email, users);
    } catch (e) {
  ...
  for (const item of QA) {
    ...
    if (!topic) {
      console.log(`Topic ${item.topicSlug} not found, skipping question`);
      continue;
    }
    ...
    try {
      ...
      const seedUser = allUsers.get("seed@theceylonx.com");
      if (!seedUser) {
        console.log("Seed user not found, skipping question creation");
        continue;
      }
